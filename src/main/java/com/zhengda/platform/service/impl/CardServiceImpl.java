...
public class CardServiceImpl implements CardService {
    ...
    public Card getByCardCode(String plantCode, String cardCode, Long employeeId) {
        if (StringUtils.isEmpty(plantCode) || StringUtils.isEmpty(cardCode) || employeeId == null) {
            return null;
        }
        CardQueryBo cardQueryBo = new CardQueryBo();
        cardQueryBo.setDeleted(Constants.DELETED_NO);
        cardQueryBo.setCardCode(cardCode);
        cardQueryBo.setPlantCode(plantCode);
        List<Card> list = this.getList(cardQueryBo);
